...
int main(int argc, char *argv[])
{
	...
	count = scandir(dirname,&dent,NULL,alphasort);
	while(count!=idx)
	{
		sprintf(pathname, "%s/%s", dirname, dent[idx]->d_name);
		...
		if(flag==1 | flag==3)
		{
			...
			group_entry->gr_name, statbuf.st_size, temp, dent[idx]->d_name);
		}
		else //flag==2 옵션은 없고 경로만 있을때
		{
			printf("%s ",dent[idx]->d_name);
		}
		idx++;
	}
	if(flag==2||flag==0)	printf("\n"); //dir_entry 만 출력해서 마지막에 \n작업이 필요한경우
